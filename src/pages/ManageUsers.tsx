...
import { getUsersByCompany, deleteRegularUser, type RegularUser } from '@/lib/mockAuth'
...
export default function ManageUsers() {
  ...
  const load = async () => {
    ...
    const data = await getUsersByCompany(company)
    setUsers(data)
    setLoading(false)
  }
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u.id} className="border-b">
                        ...
                        <td className="py-3 px-4 text-sm text-gray-600">{new Date(u.created_at).toLocaleDateString()}</td>
                        <td className="py-3 px-4">
